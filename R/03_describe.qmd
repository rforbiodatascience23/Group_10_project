...
cancer_dataset_clean_binary <- read_tsv("/net/pupil1/home/people/s194681/projects/Group_10_project_2/02_dat_clean_binary.tsv")
```
...
cancer_dataset_clean_binary_long <- gather(cancer_dataset_clean_binary , key = "Parameter", value = "Value", `radius mean`, `texture mean`, `perimeter mean`, `area mean`, `smoothness mean`, `concavity mean`, `concave points mean`, `symmetry mean`, `fractal dimension mean`, `compactness mean`) |>
  group_by(Parameter) |>
  mutate(StandardizedValue = scale(Value))

# Standardize the values within each level of Parameter
#cancer_dataset_clean_binary_long <- cancer_dataset_clean_binary_long |>
#  group_by(Parameter) |>
#  mutate(StandardizedValue = scale(Value))

# Customize boxplot
gg <- ggplot(data = cancer_dataset_clean_binary_long, 
...
# gather the specific parameters
# Standardize the values within each level of Parameter
cancer_dataset_clean_binary_long <- gather(cancer_dataset_clean_binary, key = "Parameter", value = "Value", `radius se`, `texture se`, `perimeter se`, `area se`, `smoothness se`, `concavity se`, `concave points se`, `symmetry se`, `fractal dimension se`, `compactness se`) |>
  group_by(Parameter) |>
  mutate(StandardizedValue = scale(Value))
...
cancer_dataset_clean_long_binary <- gather(cancer_dataset_clean_binary, key = "Parameter", value = "Value", `radius worst`, `texture worst`, `perimeter worst`, `area worst`, `smoothness worst`, `concavity worst`, `concave points worst`, `symmetry worst`, `fractal dimension worst`, `compactness worst`) |>
  group_by(Parameter) |>
  mutate(StandardizedValue = scale(Value))
