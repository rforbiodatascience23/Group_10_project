...
tidy_correlation <- correlation_matrix |>
  as_tibble() |> 
  mutate(myrownames) |>
  column_to_rownames("myrownames") |>
  pivot_longer(cols = starts_with("myrownames"), names_to = "Variable", values_to = "Value")
  #gather(key = "variable2", value = "correlation", -variable1)
print(tidy_correlation)

ggplot_heatmap <- tidy_correlation |>
  ggplot(data = _) +
